...
void texteditor::Creatslots()
{
    ...
    //菜单 --- 编辑
    connect(menu_edit_undo, &QAction::triggered, this, [this]() {
        ui->textEdit->undo();
    }); //使用QTextEdit自带的槽函数
    ...
    connect(menu_edit_delete, &QAction::triggered, this, [this]() {
        ui->textEdit->cut();
        Clipboard->setText(" ");
    });

    //菜单 --- 格式
    connect(menu_format_font, &QAction::triggered, this, &texteditor::slot_menu_Setfont);
    connect(menu_format_backgroundcolor,
            &QAction::triggered,
            this,
            &texteditor::slot_menu_Backgroundcolor);
    connect(menu_format_fontColor, &QAction::triggered, this, &texteditor::slot_menu_fontcolor);

    //菜单 --- 关于
    connect(menu_help_about, &QAction::triggered, this, &texteditor::slot_menu_About);
...
void texteditor::Menu()
{
    ...
    menu_file_New = menu_file->addAction(QIcon(":/image/file.png"), tr("新建文件"));
    menu_file_New->setShortcut(QKeySequence("Ctrl+N"));

    menu_file_Open = menu_file->addAction(QIcon(":/image/file.png"), tr("打开文件"));
    menu_file_Open->setShortcut(QKeySequence("Ctrl+O"));

    menu_file->addSeparator(); //设置间隔期
    menu_file_Save = menu_file->addAction(QIcon(":/image/save.png"), tr("保存文件"));
    ...
    menu_edit_undo = menu_edit->addAction(QIcon(":/image/Undo.png"), tr("撤销"));
    menu_edit_undo->setShortcut(QKeySequence::Undo);
    menu_edit_undo->setEnabled(false);
    menu_edit->addSeparator();

    menu_edit_cut = menu_edit->addAction(QIcon(":/image/Cut.png"), tr("剪切"));
    menu_edit_cut->setShortcut(QKeySequence::Cut);
    menu_edit_cut->setEnabled(false);

    menu_edit_copy = menu_edit->addAction(QIcon(":/image/Copy.png"), tr("复制"));
    menu_edit_copy->setShortcut(QKeySequence::Copy);
    menu_edit_copy->setEnabled(false);

    menu_edit_paste = menu_edit->addAction(QIcon(":/image/Paste.png"), tr("粘贴"));
    menu_edit_paste->setShortcut(QKeySequence::Paste);

    menu_edit_delete = menu_edit->addAction(QIcon(":/image/delete.png"), tr("删除"));
    menu_edit_delete->setShortcut(QKeySequence::Delete);
    menu_edit_delete->setEnabled(false);
    menu_edit_delete->setShortcut(QKeySequence::Delete);
    menu_edit->addSeparator();

    menu_edit_all = menu_edit->addAction(QIcon(":/image/SelectAll.png"), tr("全选"));
    ...
    menu_format_font = menu_format->addAction(QIcon(":/image/font.png"), tr("字体"));
    menu_format_font->setShortcut(QKeySequence("Ctrl+F"));
    menu_format_font->setCheckable(true); //设置选中状态

    menu_format_fontColor = menu_format->addAction(QIcon(":/image/fontcolor.png"), tr("字体颜色"));
    menu_format_fontColor->setCheckable(true); //设置选中状态

    menu_format_backgroundcolor = menu_format->addAction(QIcon(":/image/backcolor.png"), tr("背景颜色"));
    ...
    menu_help_about = menu_help->addAction(QIcon(":/image/about.png"), tr("关于记事本"));
...
{
    const QByteArray cpath = filename.toLocal8Bit();
...
{
    QString path = QFileDialog::getSaveFileName(this, tr("打开文件"), ".", tr("文本文件(*.txt)"));
    if (!path.isEmpty()) {
        QFile file(path);
        if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            QMessageBox::warning(this, tr("保存文件"), tr("不能打开文件:\n%1").arg(path));
            return;
        }
...
{
    QColor color = QColorDialog::getColor(Qt::red);
    QString sty = QString("background-color:%1").arg(color.name()); //动态获取背景颜色
    ui->textEdit->setStyleSheet(sty);                               //动态修改颜色
}
